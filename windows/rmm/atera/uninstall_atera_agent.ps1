Function get_uninstall_codes ([string]$DisplayName) {
...
Function get_product_keys ([string]$ProductName) {
...
Function get_service_status ([string]$Name) { (Get-Service -Name $Name -ErrorAction SilentlyContinue).Status }

Function stop_running_services ([string]$Name) {
If ( $(get_service_status -Name $Name) -eq "Running" ) { Write-Output "Stopping : ${Name} service" ; Stop-Service -Name $Name -Force }
}

Function remove_stopped_services ([string]$Name) {
$s = (get_service_status -Name $Name)
If ( $s ) {
If ( $s -eq "Stopped" ) {
...
Function stop_running_processes ([string]$Name) {
...
Function remove_path ([string]$Path) {
If ( Test-Path $Path ) {
...
Function get_exe_files ([string]$Path) {
If ( Test-Path $Path ) {
...
'AteraAgent', 'Splashtop for RMM', 'Splashtop Streamer' | ForEach-Object { get_uninstall_codes -DisplayName $_ }

# Get product keys from the list of installed products
$ProductKeys = New-Object System.Collections.ArrayList
'AteraAgent', 'Splashtop for RMM', 'Splashtop Streamer' | ForEach-Object { get_product_keys -ProductName $_ }
...
"${Env:ProgramFiles}\ATERA Networks" | ForEach-Object { get_exe_files -Path $_ }
...
$ServiceList | ForEach-Object { stop_running_services -Name $_ }

# Terminate all relevant processes that may still be running
$ExeFiles.Add('reg') | Out-Null
$ExeFiles | ForEach-Object { stop_running_processes $_ }

# Delete services if they're still present
$ServiceList | ForEach-Object { remove_stopped_services -Name $_ }

# Delete products from MSI installer registry
$ProductKeys | ForEach-Object { remove_path -Path "HKCR:Installer\Products\${_}" }

# Unmount HKEY_CLASSES_ROOT registry hive
Remove-PSDrive -Name HKCR

# Delete registry keys
$RegistryKeys | ForEach-Object { remove_path -Path $_ }

# Delete remaining directories
#Write-Host "Waiting for file locks to be freed" ; Start-Sleep -Seconds 4
$Directories | ForEach-Object { remove_path -Path $_ }